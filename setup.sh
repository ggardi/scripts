...
install_php() {
    ...
    if command_exists php; then
        ...
        print_info "Current PHP version: $CURRENT_PHP_VERSION"
        ...
        if [[ "$CURRENT_PHP_VERSION" == "$PHP_VERSION" ]]; then
            ...
            print_warning "PHP $CURRENT_PHP_VERSION is installed, but we need PHP $PHP_VERSION"
...
configure_php_alternatives() {
    # Set PHP 8.1 as default if multiple versions exist
    if command_exists update-alternatives; then
        # Check if PHP alternatives are configured
        if ! update-alternatives --query php >/dev/null 2>&1; then
            print_info "Configuring PHP alternatives..."
            sudo update-alternatives --install /usr/bin/php php /usr/bin/php$PHP_VERSION 100
        fi
        sudo update-alternatives --set php /usr/bin/php$PHP_VERSION
        print_info "Set PHP $PHP_VERSION as default"
    else
        # Install update-alternatives if not available
        print_info "Installing update-alternatives..."
        refresh_sudo
