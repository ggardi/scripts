...
### Skip Options
```bash
# Skip PHP installation (if you already have PHP 8.1)
./setup.sh --skip-php
...
2. **PHP 8.1 Installation**
   - Adds Ondrej Sury's PHP PPA (reliable PHP repository)
...
3. **Composer Installation**
   - Downloads and verifies Composer installer
   - Installs Composer globally

4. **Environment Configuration**
   - Creates `.env.environment` file with "local" environment
   - Copies `.env.local` to `.env` for local development configuration
   - If `.env.local` doesn't exist, uses `.env.example`
...
- `.env.local` - Local development template (SQLite, file caching)
- `.env.example` - Fallback template if `.env.local` doesn't exist

The setup script will:
1. Create `.env.environment` with "local"
2. Copy `.env.local` to `.env` for your configuration
